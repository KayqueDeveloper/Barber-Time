...
export const FormatarSalario = (
  salario: number | string | null | undefined
): string => {
  if (!salario) return ""; // Retorna vazio caso o valor seja inválido
  const valorNumerico =
    typeof salario === "string"
      ? parseFloat(salario.replace(/\D/g, "")) / 100
      : salario;
